...
def novo_membro():
    ...
    # 1. Validação de documento duplicado
    if num_doc:
        existente_doc = conn.execute("SELECT id, nome FROM membros WHERE numero_documento = ? AND numero_documento != ''", (num_doc,)).fetchone()
        if existente_doc:
            conn.close()
            session['alerta_duplicado'] = f"Atenção: Já existe um membro registado com o documento nº {num_doc} ({existente_doc['nome']}). Registo cancelado para evitar duplicidade."
            return redirect(url_for('dashboard'))

    # 2. Validação de nome idêntico duplicado
    existente_nome = conn.execute("SELECT id FROM membros WHERE LOWER(TRIM(nome)) = LOWER(?)", (nome,)).fetchone()
    if existente_nome:
        conn.close()
        session['alerta_duplicado'] = f"Atenção: O membro '{nome}' já se encontra registado no sistema. Não foi duplicado."
        return redirect(url_for('dashboard'))
...
# GESTÃO COMPLETA DE UTILIZADORES (ADMIN / PASTOR)
...
def novo_usuario():
    ...
    if user and senha:
        ...
        try:
            ...
            conn.commit()
            session['sucesso_cadastro'] = f"Utilizador '{user}' ({cargo}) criado com sucesso!"
        except sqlite3.IntegrityError:
            session['alerta_duplicado'] = f"O utilizador '{user}' já existe. Escolha outro nome de login."
        conn.close()
    return redirect(url_for('dashboard'))
...
def apagar_usuario(id):
    if not is_admin(): return redirect(url_for('dashboard'))
    conn = get_db()
    user = conn.execute("SELECT usuario FROM usuarios WHERE id = ?", (id,)).fetchone()
    if user and user['usuario'] != 'admin':
        conn.execute("DELETE FROM usuarios WHERE id = ?", (id,))
        conn.commit()
        session['sucesso_cadastro'] = f"Utilizador '{user['usuario']}' removido com sucesso."
    conn.close()
    return redirect(url_for('dashboard'))
