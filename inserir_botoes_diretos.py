    ...
    <main class="max-w-7xl mx-auto p-3.5 sm:p-5 lg:p-6 space-y-6">
        ...
        <section id="aba-membros" class="tab-content {% if pode_cadastro %}active{% endif %} space-y-6">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div class="lg:col-span-5 bg-white/95 rounded-3xl card-glow border border-indigo-100 p-5 sm:p-6">
                    ...
                    <form action="/membros/novo" method="POST" enctype="multipart/form-data" class="space-y-5">
                        <div class="flex flex-col items-center justify-center p-5 bg-gradient-to-b from-blue-50/60 to-indigo-50/40 border-2 border-dashed border-indigo-200 rounded-3xl relative group hover:border-blue-500 transition duration-200">
                            ...
                            <p class="text-[11px] text-slate-500 font-semibold mt-2">Fotografia do membro</p>
                        </div>
                ...
                <!-- Tabela de Membros com Todos os Botões Diretos -->
                    ...
                    <div>
                        <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 mb-5 pb-4 border-b border-indigo-50">
                            <div>
                                ...
                                <p class="text-xs text-slate-500 font-medium">Cartões, Certificados e WhatsApp</p>
                            </div>
                            <div>
                        ...
                        <div class="overflow-x-auto max-h-[640px] rounded-2xl border border-indigo-50">
                            <table id="tabelaMembros" class="w-full text-left text-sm min-w-[700px]">
                                <thead class="bg-gradient-to-r from-blue-900 to-indigo-950 text-white font-black text-xs uppercase tracking-wider sticky top-0 z-10">
                                    <tr>
                                        ...
                                        <th class="p-3.5">Posição / Depto</th>
                                        <th class="p-3.5 text-center">Cartão & Contato</th>
                                        <th class="p-3.5 text-center">Certificados (PDF)</th>
                                ...
                                <tbody class="divide-y divide-slate-100 text-slate-700 bg-white">
                                    ...
                                    <tr class="hover:bg-blue-50/60 transition">
                                        ...
                                        <td class="p-3.5">
                                            <span class="bg-blue-50 border border-blue-200 text-blue-900 font-black px-2.5 py-1 rounded-xl text-xs inline-block">
                                                {{ m['posicao_atual'] or 'Membro' }}
                                            </span>
                                            <span class="text-xs text-slate-500 font-bold block mt-1">
                                                {{ m['departamento'] or 'Geral' }}
                                            </span>
                                        </td>
                                        <!-- BOTÕES VISÍVEIS: CARTÃO AZUL E WHATSAPP VERDE -->
                                        <td class="p-3.5 text-center whitespace-nowrap">
                                            <div class="flex items-center justify-center gap-1.5">
                                                <!-- BOTÃO AZUL: BAIXAR CARTÃO (PDF) -->
                                                <a href="/membro/cartao_pdf/{{ m['id'] }}" title="Baixar Cartão Oficial em PDF" class="bg-blue-600 hover:bg-blue-700 text-white font-extrabold px-3 py-1.5 rounded-xl text-xs shadow-md transition flex items-center gap-1">
                                                    <span>🪪</span> <span>Cartão (PDF)</span>
                                                </a>
                                                <!-- BOTÃO VERDE: FALAR NO WHATSAPP -->
                                                {% if m['telefone'] %}
                                                <a href="https://wa.me/258{{ m['telefone']|replace(' ', '')|replace('+', '')|replace('-', '') }}" target="_blank" title="Conversar no WhatsApp" class="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold px-3 py-1.5 rounded-xl text-xs shadow-md transition flex items-center gap-1">
                                                    <span>💬</span> <span>WhatsApp</span>
                                                </a>
                                                {% else %}
                                                <span class="text-[10px] text-slate-400 font-medium italic">Sem contacto</span>
                                                {% endif %}
                                            </div>
                                        </td>
                                        <!-- OS TRÊS BOTÕES BRANCOS DE CERTIFICADOS -->
                                        <td class="p-3.5 text-center whitespace-nowrap">
                                            <div class="inline-flex rounded-xl shadow-sm border border-slate-200 overflow-hidden bg-white">
                                                <a href="/membro/certificado_pdf/{{ m['id'] }}/batismo" title="Emitir Certificado de Batismo" class="px-2.5 py-1.5 text-xs font-bold text-blue-900 bg-white hover:bg-blue-50 border-r border-slate-200 transition">
                                                    Batismo
                                                </a>
                                                <a href="/membro/certificado_pdf/{{ m['id'] }}/apresentacao" title="Emitir Certificado de Apresentação" class="px-2.5 py-1.5 text-xs font-bold text-emerald-900 bg-white hover:bg-emerald-50 border-r border-slate-200 transition">
                                                    Apresentação
                                                </a>
                                                <a href="/membro/certificado_pdf/{{ m['id'] }}/recomendacao" title="Emitir Carta de Recomendação" class="px-2.5 py-1.5 text-xs font-bold text-purple-900 bg-white hover:bg-purple-50 transition">
                                                    Recomendação
                                                </a>
                                            </div>
                                        </td>
                                        <td class="p-3.5 text-center whitespace-nowrap">
                                            {% if e_admin %}
                                            <a href="/apagar/membro/{{ m['id'] }}" onclick="return confirm('Eliminar o membro definitivamente?');" class="text-rose-600 hover:text-rose-800 font-black text-xs p-1">✕</a>
                                            {% endif %}
                                        </td>
                                    </tr>
                                    {% endfor %}
