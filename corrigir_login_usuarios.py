...
# 1. Garantir que a rota /login trate espaços em branco e redirecione com segurança
bloco_login_antigo = """@app.route('/login', methods=['GET', 'POST'])
def login():"""

# 2. Garantir que a rota /estudos exista e não quebre
rota_estudos_segura = """
@app.route('/estudos')
def portal_estudos():
...
# Reescrever o app.py garantindo que todas as variáveis do template dashboard existam com valores padrão seguros
codigo_corrigido = """from flask import Flask, render_template, request, redirect, url_for, session, send_file
import sqlite3
...
print("✓ app.py corrigido com tratamento seguro de login para todos os perfis!")