...
<body class="bg-gradient-to-br from-slate-100 via-blue-50/50 to-indigo-50/40 text-slate-900 min-h-screen antialiased selection:bg-blue-600 selection:text-white">
    ...
    <header class="bg-gradient-to-r from-blue-950 via-indigo-950 to-slate-950 text-white shadow-xl sticky top-0 z-40 border-b border-indigo-500/20">
        <div class="max-w-7xl mx-auto px-4 py-3.5 flex justify-between items-center">
            ...
            <div class="flex items-center space-x-2.5">
                ...
                <a href="/logout" class="bg-gradient-to-r from-rose-500 to-red-600 hover:from-rose-600 hover:to-red-700 active:scale-95 text-white text-xs font-extrabold px-3.5 py-2 rounded-xl shadow-md shadow-rose-900/30 transition duration-150">
                    Sair
                </a>
            </div>
        </div>
                            ...
                            <table id="tabelaMembros" class="w-full text-left text-sm min-w-[620px]">
                                <thead class="bg-gradient-to-r from-blue-900 to-indigo-950 text-white font-black text-xs uppercase tracking-wider sticky top-0 z-10">
                                    <tr>
                                        ...
                                        <th class="p-3.5 text-center">Ações / Documentos</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100 text-slate-700 bg-white">
                                    ...
                                    <tr class="hover:bg-blue-50/60 transition">
                                        ...
                                        <td class="p-3.5 text-center whitespace-nowrap space-x-1">
                                            <!-- Botão Ficha -->
                                            <button onclick="verCartaoMembro({{ m['id'] }})" class="bg-blue-600 hover:bg-blue-700 text-white font-extrabold px-3 py-1.5 rounded-xl text-xs shadow-sm transition active:scale-95">
                                                Ficha
                                            </button>
                                            <!-- Botão Direto Cartão PDF -->
                                            <a href="/membro/cartao_pdf/{{ m['id'] }}" title="Baixar Cartão Oficial em PDF" class="bg-indigo-700 hover:bg-indigo-800 text-white font-extrabold px-3 py-1.5 rounded-xl text-xs shadow-sm transition inline-block">
                                                🪪 Cartão
                                            </a>
                                            {% if e_admin %}
                            ...
                            <tbody class="divide-y divide-slate-100 bg-white">
                                ...
                                <tr class="hover:bg-emerald-50/40 transition">
                                    ...
                                    <td class="p-3 text-center">
                                        <a href="/financeiro/recibo_pdf/{{ f['id'] }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-2 py-1 rounded text-xs">📄 PDF</a>
                                    </td>
                                    {% if e_admin %}
    ...
    <!-- Modal Cartão de Membro Oficial com Todos os Botões Integrados -->
    <div id="modalMembro" class="fixed inset-0 bg-slate-950/70 backdrop-blur-sm hidden items-center justify-center p-4 z-50">
        <div class="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl relative border border-slate-200 max-h-[90vh] overflow-y-auto">
    ...
    <script>
        ...
        function verCartaoMembro(id) {
            ...
            document.getElementById('conteudoModalMembro').innerHTML = `
                ...
                <!-- Botões de Ação Imediata: Cartão PDF e WhatsApp -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    <a href="/membro/cartao_pdf/${m.id}" class="h-12 bg-blue-700 hover:bg-blue-800 text-white font-black text-xs rounded-2xl shadow-md shadow-blue-700/30 flex items-center justify-center gap-2 transition">
                        <span>🪪</span> <span>Baixar Cartão (PDF)</span>
                    </a>
                    <a href="${linkWhats}" target="_blank" class="h-12 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-2xl shadow-md shadow-emerald-600/30 flex items-center justify-center gap-2 transition">
                        <span>💬</span> <span>Falar no WhatsApp</span>
                    </a>
                </div>

                <!-- Emissão de Certificados Pastorais em PDF -->
                <div class="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl space-y-2">
                    <span class="text-xs font-black text-slate-800 uppercase tracking-wider block">📜 Emitir Certificados Pastorais (PDF)</span>
                    <div class="grid grid-cols-3 gap-2">
                        <a href="/membro/certificado_pdf/${m.id}/batismo" class="py-2.5 bg-white hover:bg-blue-50 border border-blue-200 text-blue-900 text-center font-bold text-xs rounded-xl shadow-sm transition">
                            Batismo
                        </a>
                        <a href="/membro/certificado_pdf/${m.id}/apresentacao" class="py-2.5 bg-white hover:bg-emerald-50 border border-emerald-200 text-emerald-900 text-center font-bold text-xs rounded-xl shadow-sm transition">
                            Apresentação
                        </a>
                        <a href="/membro/certificado_pdf/${m.id}/recomendacao" class="py-2.5 bg-white hover:bg-purple-50 border border-purple-200 text-purple-900 text-center font-bold text-xs rounded-xl shadow-sm transition">
                            Recomendação
                        </a>
                    </div>
                </div>
