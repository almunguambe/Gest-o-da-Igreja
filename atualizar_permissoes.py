...
# -------------------------------------------------------------
# 1. TEMPLATE COM CONTROLO DE ABAS POR PERFIL DE UTILIZADOR
# -------------------------------------------------------------
...
<body class="bg-slate-100 text-slate-800 font-sans min-h-screen">
    ...
    <header class="bg-gradient-to-r from-blue-950 via-blue-900 to-indigo-900 text-white shadow sticky top-0 z-40">
        ...
        <nav class="max-w-7xl mx-auto px-4 flex overflow-x-auto space-x-2 border-t border-blue-800/60 pt-2 pb-1 text-xs">
            ...
            {% if pode_tesouraria %}
            <button onclick="trocarAba('dashboard')" id="btn-dashboard" class="tab-btn px-4 py-2 font-bold rounded-t-lg {% if not pode_cadastro %}bg-white text-blue-900 shadow{% else %}text-blue-200 hover:bg-blue-800/50{% endif %}">📊 Painel Geral</button>
                ...
                <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
                    ...
                    <form action="/membros/novo" method="POST" enctype="multipart/form-data" class="space-y-4 text-xs">
                        <div class="p-3 bg-slate-50 border border-dashed border-slate-300 rounded-xl text-center">
                            <label class="block font-bold text-slate-700 mb-1">Fotografia do Membro</label>
                ...
                <div class="lg:col-span-2 bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs">
                            <thead class="bg-slate-50 border-b"><tr><th class="p-2">Data</th><th class="p-2">Noivos</th><th class="p-2">Oficiante</th>{% if e_admin %}<th class="p-2 text-center">Ação</th>{% endif %}</tr></thead>
                            <tbody class="divide-y divide-slate-100">
                                {% for c in todos_casamentos %}
                                <tr>
                                    <td class="p-2">{{ c['data_casamento'] }}</td><td class="p-2 font-bold">{{ c['noivo'] }} & {{ c['noiva'] }}</td><td class="p-2">{{ c['pastor_oficiante'] or '-' }}</td>
                                    {% if e_admin %}
                                    <td class="p-2 text-center"><a href="/apagar/casamento/{{ c['id'] }}" onclick="return confirm('Eliminar?');" class="text-rose-600 font-bold">X</a></td>
                                    {% endif %}
                                </tr>
                                {% endfor %}
                            </tbody>
        ...
        <section id="aba-mortes" class="tab-content space-y-6">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs">
                            <thead class="bg-slate-50 border-b"><tr><th class="p-2">Data</th><th class="p-2">Nome</th><th class="p-2">Notas</th>{% if e_admin %}<th class="p-2 text-center">Ação</th>{% endif %}</tr></thead>
                            <tbody class="divide-y divide-slate-100">
                                {% for m in todas_mortes %}
                                <tr>
                                    <td class="p-2">{{ m['data_falecimento'] }}</td><td class="p-2 font-bold">{{ m['nome_falecido'] }}</td><td class="p-2">{{ m['observacoes'] or '-' }}</td>
                                    {% if e_admin %}
                                    <td class="p-2 text-center"><a href="/apagar/morte/{{ m['id'] }}" onclick="return confirm('Eliminar?');" class="text-rose-600 font-bold">X</a></td>
                                    {% endif %}
                                </tr>
                                {% endfor %}
                            </tbody>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                    <div class="flex justify-between items-center mb-3 pb-2 border-b">
                        <h3 class="text-sm font-bold uppercase">Movimentos Registados</h3>
                        <a href="/exportar/financeiro" class="text-xs bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg font-bold">📥 Exportar Tesouraria (.xlsx)</a>
                    </div>
                    <div class="overflow-x-auto max-h-[500px]">
                        <table class="w-full text-left text-xs">
                            <thead class="bg-slate-50 border-b sticky top-0"><tr><th class="p-2">Data</th><th class="p-2">Tipo</th><th class="p-2">Conta</th><th class="p-2">Fundo</th><th class="p-2">Categoria</th><th class="p-2">Descrição</th><th class="p-2 text-right">Valor</th>{% if e_admin %}<th class="p-2 text-center">Ação</th>{% endif %}</tr></thead>
                            <tbody class="divide-y divide-slate-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 h-fit">
                    ...
                    <form action="/usuarios/novo" method="POST" class="space-y-3 text-xs">
                        ...
                        <label class="block font-bold text-slate-700">Cargo / Nível de Acesso *</label>
                        <select name="cargo" required class="w-full p-2 border rounded-lg bg-white font-medium">
                            <option value="Secretário">Secretário (Apenas Cadastro de Membros/Eventos)</option>
                            <option value="Líder">Líder de Ministério (Apenas Cadastro)</option>
                            <option value="Tesoureiro">Tesoureiro (Apenas Caixa, Banco e Dashboard)</option>
                            <option value="Pastor">Pastor (Acesso Total / Administrador)</option>
                        </select>
                ...
                <div class="md:col-span-2 bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs">
                            <thead class="bg-slate-50 border-b"><tr><th class="p-2">Login</th><th class="p-2">Cargo / Permissão</th><th class="p-2 text-center">Ação</th></tr></thead>
                            <tbody class="divide-y divide-slate-100">
                                ...
                                <tr>
                                    ...
                                    <td class="p-2">
                                        <span class="px-2 py-0.5 rounded text-[11px] font-bold 
                                            {% if 'Pastor' in u['cargo'] %}bg-blue-100 text-blue-800
                                            {% elif 'Tesoureiro' in u['cargo'] %}bg-emerald-100 text-emerald-800
...
print("✓ 1/2: templates/dashboard.html atualizado com permissões!")

# -------------------------------------------------------------
# 2. BACKEND FLASK COM VALIDAÇÃO DE PERMISSÕES POR ROTA
# -------------------------------------------------------------
...
with open("app.py", "w", encoding="utf-8") as f:
    f.write(app_code)
print("✓ 2/2: app.py atualizado com controlo de acessos por cargo!")